			...
			<div id="container">
				...
				<div id="outbox">
					...
					<div id="contents">
						<div id="mypage">
							<h2>
								<strong>희망도서신청</strong><span>읽고 싶은 책이 도서관에 없다면 신청해 주세요. </span>
							</h2>

							<div class="orderDivMt">
								<table summary="NO, 제목, 등록일, 조회수 순으로 이벤트 당첨자을 조회 하실수 있습니다."
									...
									<tbody>
										<c:forEach var="dto" items="${map.list }">
											<tr>
												<td class="tnone">${dto.hb_Seq }</td>
												<td class="left"><a href="./content_view?category=${map.category}&search=${map.search}&hb_Seq=${dto.hb_Seq }&page=${map.page}">${dto.hb_Title }</a></td>
												<td class="tnone">${dto.hb_User }</td>
												<td>${dto.hb_Date }</td>
												<td class="tnone right">${dto.hb_Hit }</td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
							...
							<!-- Btn Area -->
								<div class="btnArea">
									<div class="bRight">
										<ul>
											<li><a href="/write_view?hb_User=사만다" class="sbtnMini mw">글쓰기</a></li>
										</ul>
									</div>
								</div>
							<!-- //Btn Area -->
							...
							<div class="btnAreaList">
								...
								<div class="allPageMoving1">
									<a
										href="./list?category=${map.category}&search=${map.search}&page=1"
										class="n"><img src="../images/btn/btn_pre2.gif" alt="처음으로" /></a>
									<c:choose>
										...
										<c:otherwise>
											<a
												href="./list?category=${map.category}&search=${map.search}&page=${map.page-1 }" class="pre">
												<img
										src="../images/btn/btn_pre1.gif" alt="앞페이지로" /></a>
										</c:otherwise>
									</c:choose>
								...
								<c:forEach var="nowpage" begin="${map.startpage }"
									...
									<c:choose>
										<c:when test="${map.page==nowpage }">
											<li class="allPageMoving1">${nowpage }</li>
										</c:when>
										<c:otherwise>
											<a
												href="./list?category=${map.category}&search=${map.search}&page=${nowpage }" class="allPageMoving1">
												>${nowpage }</a>
										</c:otherwise>
									</c:choose>
								</c:forEach>
								<!--번호반복 -->
									...
									<c:when test="${map.page >= map.maxpage }">
										...
										src="../images/btn/btn_next1.gif" alt="뒤페이지로" /></a>
									</c:when>
									<c:otherwise>
									<a href="./list?category=${map.category}&search=${map.search}&page=${map.page+1 }" class="next"><img
										src="../images/btn/btn_next1.gif" alt="뒤페이지로" /></a>
									</c:otherwise>
								</c:choose>
...
<!-- 										src="../images/btn/btn_next1.gif" alt="뒤페이지로" /></a> -->
										...
										<a href="./list?category=${map.category}&search=${map.search}&page=${map.maxpage }"
									...
									<ul>
										...
										<form action="./list" name="search" method="post">
											...
											<select name="category" id="category">
												...
												<li><input type="text" name="search" value="${map.search }" class="searchInput" /></li>
												<li class="web"><button type="submit"><img
														src="../images/btn/btn_search.gif" alt="검색" /></button></li>
												<li class="mobile"><button type="submit"><img
														src="../images/btn/btn_search_m.gif" alt="검색" /></button></li>
										</form>
									</ul>
								</div>
							</div>
